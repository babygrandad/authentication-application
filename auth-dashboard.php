...
//search functions
if(isset($_POST['authorsSearchButton'])){
    $searchValue = mysqli_real_escape_string($conn,$_POST['authorsSearchBox']);
    $searchString = "SELECT * FROM `authors` WHERE authors.author_name LIKE '%$searchValue%'";
...
// sort functions  "SELECT * FROM `authors` ORDER by `author_name` ASC";
    ...
    <main class="form-main centering padding container-fluid" id="dataMain">
                                ...
                                <div id="flush-collapseOne" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlush">
                                    <div class="accordion-body">
                                        <a href="dashboard.php">
                                            <button type="button" class="btn btn-secondary mb-2 w-100">Books Table</button>
                                        </a>
                                        <a href="crudpages/addbook.php">
                                        <button type="button" class="btn btn-secondary mb-2 mt-2 w-100">Add New Book</button>
                                        </a>
                                    </div>
                                </div>
                            </div>
        ...
        <section class="form-containers container row">
            <div class="m-auto col-lg-8">
                <h1>Welcome to Obey Library - Authors</h1>
                <div class="col-12 searchButtonContainer" id="auth-search-container">
                    <form class="d-bolck lookUpFunctions" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
                        <div class="d-flex searchInputs">
                            <input class="form-control" type="text" name="authorsSearchBox" id="authorsSearchBox" value="<?php echo isset($_POST['authorsSearchBox']) ? $_POST['authorsSearchBox'] : '' ;?>">
                            <input class="btn" type="submit" name="authorsSearchButton" value="Search">
                        </div>
                            ...
                            while ($row = $queryResult->fetch_assoc()) {
                             ...
                             ';
                                if ($_SESSION['role'] == 'librarian') {?>
                                    ...
                                    <td>
                                    <a href="./crudpages/editauthor.php?id=<?= $row['author_id']?>">
                                        <input class="m-1 btn btn-success" type="button" value="Edit">
                                    </a>
                                        <a href="./crudpages/deleteauthor.php?id=<?= $row['author_id']?>">
